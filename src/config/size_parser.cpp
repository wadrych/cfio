...
size_t SizeParser::Parse(const std::string& size_str, 
  ...
  if (!std::isdigit(static_cast<unsigned char>(size_str[0]))) {
    throw std::invalid_argument("size string must start with a digit: '" +
                                size_str + "'");
  }
  ...
  while (pos < size_str.size() &&
         std::isdigit(static_cast<unsigned char>(size_str[pos]))) {
    ++pos;
  }
    ...
    case 'G':
      if (allowed == AllowedSuffixes::kKM) {
        throw std::invalid_argument(
            "suffix 'g' not allowed for this field: '" + size_str + "'");
      }
      multiplier = kGiga;
      break;
    default:
      throw std::invalid_argument("unknown size suffix '" +
                                  std::string(1, suffix) + "' in: '" +
                                  size_str + "'");
  }
